...
class Upload {
	protected $_uploaded = array();
 	...
 	public function insert($filePath, $fileName, $fileType, $fileSize) { 		
 		$user = new User();
		$data = new Images();
		$userID = null;
		$albumID = null;
		...
		$data->newImage("/skolaverkefni/Photobase/uploads/" . $fileName,
						$fileName,
						"Description",
						$userID,
						$albumID,
 	...
 	protected function processFile($filename, $error, $size, $type, $tmp_name, $overwrite) { 		
 		$validate = $this->checkError($filename, $error); 
 		if ($validate) {
 			$validateSize = $this->checkSize($filename, $size); 
 			$validateType = $this->checkType($filename, $type); 
 			if ($validateSize && $validateType) {
				...
				$name);
		 		if ($success) {
		 			...
		 			$this->insert($this->_destination, $name, $type, $size);
		 			if ($this->_renamed) {
